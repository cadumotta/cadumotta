...
class Biblioteca:
    ...
    def exibir_info(self):
        ...
        info += '\n'.join([livro.exibir_info() for livro in self.__livros]) or "Nenhum livro cadastrado"
        info += "\n\nUsuários Cadastrados:\n"
        info += '\n'.join([usuario.exibir_info() for usuario in self.__usuarios]) or "Nenhum usuário cadastrado"
        return info

    def salvar_em_arquivo(self, arquivo):
        dados = {
            "nome": self.__nome,
            "livros": [{"titulo": livro.titulo, "autor": livro.autor, "ano": livro.ano} for livro in self.__livros],
            "usuarios": [{
                "nome": usuario.nome,
                "idade": usuario.idade,
    ...
    def carregar_de_arquivo(arquivo):
        ...
        for livro_dados in dados["livros"]:
            biblioteca.adicionar_livro(Livro(livro_dados["titulo"], livro_dados["autor"], livro_dados["ano"]))

        for usuario_dados in dados["usuarios"]:
            usuario = Usuario(usuario_dados["nome"], usuario_dados["idade"], usuario_dados["matricula"])
    ...
    def adicionar_usuario_interativo(self):
    ...
    def adicionar_livro_interativo(self):
...
while True:
    ...
    if opcao == "1":
        biblioteca.adicionar_livro_interativo()
    elif opcao == "2":
        biblioteca.adicionar_usuario_interativo()
    elif opcao == "3":
